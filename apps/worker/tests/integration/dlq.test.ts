...
describe("RES-03: DLQ Postgres mirror — exhausted job -> dlq_events row", () => {
  ...
  it(
    "writes exactly one dlq_events row with all required fields on job exhaustion",
    { timeout: 20_000 },
    async () => {
      ...
      const count = await prisma.deadLetterEvent.count({ where: { fingerprint } });
      expect(count).toBe(1);

      // Assert all captured fields
      const row = await prisma.deadLetterEvent.findFirst({ where: { fingerprint } });
  ...
  it(
    "does NOT write a dlq_events row for an intermediate retry (not yet exhausted)",
    { timeout: 20_000 },
    async () => {
      ...
      const handler = buildDlqHandler(prisma, noopLogger);
      await handler(intermediateJob as Parameters<typeof handler>[0], new Error("transient"));
